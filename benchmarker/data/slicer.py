...
def get_span_around_middle_point(
    middle_point: int, max_bpe_seq_length: int, upper_limit: int, lower_limit: int = 0
) -> Tuple[int, int]:
    from_range = max(
        lower_limit,
        min(
...
def get_span_within_page(
    page_ranges: List[List[int]], middle_point: int, max_bpe_seq_length: int
) -> Tuple[int, int]:
    for page_range in page_ranges:
        ...
        if middle_point < page_lower_range or middle_point >= page_upper_range:
            continue
        from_range, to_range = get_span_around_middle_point(
            middle_point, max_bpe_seq_length, page_upper_range, page_lower_range
        )
        return from_range, to_range
...
class AverageJoinSlicer(BaseSlicer):
    ...
    def _generate_slice_joining_weights(
        ...
        for i, (left_overlap, right_overlap) in enumerate(
            ...
            if left_overlap > 0:
                weights[i, :left_overlap] = (
                    -self._get_averaging_weights(left_overlap) + 1
                )
            if right_overlap > 0:
    ...
    def join_predictions(
        ...
        joined_output = torch.zeros(
            total_length, model_output[0].size(-1), device=device, dtype=dtype
        )
        overlaps = [0]
        ...
        for span_wght, span_emb, span in zip(
            joining_weights, model_output, span_data  # type: ignore
        ):
            wght_span_emb = span_wght[: len(span_emb)] * span_emb
            span_length = span.end_position - span.start_position
            joined_output[span.start_position : span.end_position] += wght_span_emb[
                :span_length
            ]
