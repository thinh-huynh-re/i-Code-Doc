...
from benchmarker.cli.l5.common.pl_modules.base_lightning_module import (
    BaseLightningModule,
)
from benchmarker.cli.l5.common.utils import (
    freeze_embeds,
...
class L5GenerationModule(BaseLightningModule):
    def __init__(self, hparams: argparse.Namespace, val_metrics, **kwargs):
        ...
        self.config = load_config(model_path=Path(model_name_or_path), **vars(hparams))

        if hparams.load_ckpt_weight is not None:
            ckpt = torch.load(hparams.load_ckpt_weight, map_location="cpu")
            ckpt_state_dict = ckpt["state_dict"]
            ckpt_state_dict = {
                k[6:]: v for k, v in ckpt_state_dict.items() if k.startswith("model.")
            }
        else:
        ...
        self.setup_copying(
            self.hparams.only_copying_from_input, self.hparams.copying_exceptions
        )
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        visual_seg_data = (
            torch.tensor(get_visual_bbox(image_size).float())[None, :]
    ...
    def collect_predictions(self, data):
        ...
        if self.hparams.val_metric[0] == "anls":
            ...
            if "label_name" in data:
                for label, pred, ref in zip(
                    data["label_name"], data["preds"], data["target"]
                ):
                    if pred != "None":
    ...
    def ids_to_clean_text(self, generated_ids: List[List[int]]) -> List[str]:
        ...
        for gen_line in generated_ids:
            ...
            if (
                nz.shape[0] == 0
            ):  # It is possible there are only zeros when decoder has changed
    ...
    def _generative_step(self, batch: dict, prefix="dev") -> dict:
        ...
        if self.only_copying_from_input:
            _allowed_tokens = [
                list(
                    set(i).difference(self.always_forbidden).union(self.always_allowed)
                )
        ...
        generation_results = {
            "preds": preds,
            "target": target,
            "is_equal": [p == t for p, t in zip(preds, target)],
        }
        if "doc_id" in batch:
        ...
        base_metrics.update(
            gen_time=gen_time, gen_len=summ_len, generation_results=generation_results
        )
    ...
    def get_dataloader(
        ...
        raise NotImplementedError(
            "dataloader should be assigned from separate DataModule"
        )
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument(
            "--logger_name",
            type=str,
            choices=["default", "wandb", "wandb_shared", "mlflow"],
            default="default",
        )
        ...
        parser.add_argument(
            "--only_copying_from_input", action="store_true", default=False
        )
        parser.add_argument("--copying_exceptions", type=str, default="yes no Yes No")

        parser.add_argument(
            "--context_residual", type=str, default=None, required=False
        )
