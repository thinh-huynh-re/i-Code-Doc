...
class L5DataModule(pl.LightningDataModule):
    def __init__(self, hparams, **kwargs):
        ...
        self.data_dir = {
            "train": os.path.join(
                self.hparams.data_dir[0], self.hparams.train_data_dir[0]
            ),
            "val": os.path.join(self.hparams.data_dir[0], self.hparams.val_data_dir[0]),
            "test": os.path.join(
                self.hparams.data_dir[0], self.hparams.test_data_dir[0]
            ),
        }
        self.datasets = {}
    ...
    def setup(self, stage):
        [
            self._setup_datasets(directory, split)
            for split, directory in self.data_dir.items()
            if directory
        ]
    ...
    def _setup_datasets(self, directory: Any, split: str):
        if isinstance(directory, list):
            ...
            self.datasets[split] = (
                ConcatDataset(datasets_list) if len(directory) > 1 else datasets_list[0]
            )
        else:
    ...
    def _load_dataset(self, directory: Path, split: str) -> Dataset:
        additional_data_fields = (
            ("doc_id",) if split == "train" else self.additional_data_fields
        )
    ...
    def resize_data(self, data, split):
        ...
        if input_len > 0:
            ...
            data["seg_data"]["tokens"]["bboxes"] = self.resize_mmap(
                data["seg_data"]["tokens"]["bboxes"], input_len
            )
        if target_len > 0:
    ...
    def add_data_specific_args(parser):
        parser.add_argument(
            ...
            type=Path,
            nargs="+",
            help="The sequence of input data directories.",
        )
        parser.add_argument(
            "--datasets_weights",
            default=None,
