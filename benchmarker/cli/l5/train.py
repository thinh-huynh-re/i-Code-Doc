...
def main(
    args: Namespace,
    model: Optional[L5GenerationModule] = None,
    datamodule=None,
    callbacks=None,
    process_position=0,
) -> Tuple[L5GenerationModule, pl.Trainer]:
    ...
    trainer: pl.Trainer = get_generic_trainer(
        model, datamodule, args, callbacks, loggers=loggers
    )
...
def setup_callbacks(args: Namespace, process_position=0) -> List[pl.Callback]:
    ...
    if args.early_stopping_patience >= 0:
        ...
        es_callback = get_early_stopping_callback(
            es_metric, args.early_stopping_patience
        )
    ...
    callbacks.append(
        get_checkpoint_callback(
            args.output_dir, args.val_metric[0], args.save_top_k, lower_is_better
        )
    )
    callbacks.append(SavePredictionCallback())
    callbacks.append(
        SaveTransformerCheckpoint(save_path=Path(args.output_dir) / "best_tfmr")
    )
...
def setup_loggers(args: Namespace) -> List[pl.loggers.LightningLoggerBase]:
    ...
    if (
        ...
        loggers.append(
            BetterMlFlowLogger(
                args.mlflow_experiment, args.mlflow_uri, args.mlflow_tags
            )
        )
    else:
...
def get_generic_trainer(
    model: L5GenerationModule,
    datamodule: L5DataModule,
    args: Namespace,
