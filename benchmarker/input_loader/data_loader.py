...
class DataLoader(IterBaseClass, Generic[T], metaclass=ABCMeta):
    def __init__(
        self, docs: Iterable[T], segment_levels: Optional[Sequence[str]] = None
    ) -> None:
        ...
        self._segment_levels: Sequence[str] = tuple(
            set(
                (list(segment_levels) if segment_levels else [])
                + list(self._mandatory_levels)
            )
        )
