...
MODEL_CLASSES: Dict[
    str, Tuple[UdopConfig, T5ForConditionalGeneration, UdopTokenizer]
] = {
...
def main():
    ...
    #     data_args: DataTrainingArguments = args[1]
    #     training_args: TrainingArguments = args[2]
    # else:
    ...
    if model_args.model_type in MODEL_CLASSES:
        ...
        config_type, model_type, tokenizer_type = (
            AutoConfig,
            AutoModelForTokenClassification,
            AutoTokenizer,
        )
    ...
    # Get datasets
    train_dataset = (
        RvlCdipDataset(data_args=data_args, tokenizer=tokenizer, mode="train")
        if training_args.do_train
        else None
    )
    # TODO: for now use use test dataset for all evaluation -- will use both later
    eval_dataset = (
        RvlCdipDataset(data_args=data_args, tokenizer=tokenizer, mode="test")
