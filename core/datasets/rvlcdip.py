...
class RvlCdipDataset(Dataset):
    ...
    def __init__(self, data_args, tokenizer, mode="train"):
        ...
        ocr_dir = os.path.join(
            data_args.data_dir, data_args.mpdfs_dir, "cdip-images-full-clean-ocr021121"
        )
        ...
        results = [
            self.load_file(filepath, ocr_dir, image_dir) for filepath in tqdm(file_list)
        ]
    ...
    def __getitem__(self, index):
        try:
            ...
            rets, n_split = read_ocr_core_engine(
                self.examples[index],
                self.images[index],
                self.tokenizer,
                self.max_seq_length,
            ...
            for i in range(n_split):
                ...
                input_ids, labels, bbox_input = self.cls_collator(
                    input_ids, bbox, label
                )
                ...
                decoder_attention_mask = torch.tensor(
                    decoder_attention_mask, dtype=torch.long
                )
    ...
    def pad_tokens(self, input_ids, bbox):
        ...
        start_token, _, end_token = (
            tokenized_tokens[0],
            tokenized_tokens[1:-1],
            tokenized_tokens[-1],
        )
...
def read_ocr_core_engine(
    file, image_dir, tokenizer, max_seq_length, num_img_embeds, image_size
):
