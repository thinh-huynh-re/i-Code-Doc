...
def save_visualize_mae(
    image_output_, image_target, image_mask_label=None, output_dir=None
):
    ...
    for k in range(len(image_output)):
        im_o = np.transpose(
            np.clip(
                image_output[k].float().detach().cpu().numpy() * 255.0, 0.0, 255.0
            ).astype(np.uint8),
            (1, 2, 0),
        )
        im_t = np.transpose(
            np.clip(
                image_target[k].float().detach().cpu().numpy() * 255.0, 0.0, 255.0
            ).astype(np.uint8),
            (1, 2, 0),
        )
        im_o_masked = np.transpose(
            np.clip(
                image_output_masked[k].float().detach().cpu().numpy() * 255.0,
                0.0,
                255.0,
            ).astype(np.uint8),
            (1, 2, 0),
        ...
        Image.fromarray(im_o_masked).save(
            os.path.join(save_dir, f"output_masked_{str(k)}.jpg")
        )
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.sampler.Sampler]:
        ...
        if self.args.world_size <= 1:
            ...
            for key in self.train_dataset:
                samplers[key] = RandomSampler(
                    self.train_dataset[key], generator=generator
                )
            return samplers
            ...
            for key in self.train_dataset:
                samplers[key] = (
                    RandomSampler(self.train_dataset[key])
