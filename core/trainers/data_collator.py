    ...
    def __call__(
        ...
        if "position_ids" not in batch:
            position_ids = torch.stack(
                [torch.arange(target_len, dtype=torch.long) for _ in range(batch_size)]
            )
        ...
        if "image" in features[0]:
            ...
            for k in ["image_mask_label"]:
                if k in features[0] and features[0][k] is not None:
                    ...
                    ids_keeps = []
                    for d in features:
                        mask, ids_restore, ids_remove, ids_keep = random_masking(
                            int(image_size[2] ** 2 / 16**2), mask_ratio
                        )
