...
class T52dStack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if image is not None:
            vision_embeds = self.vision_encoder.forward_encoder(
                image, ids_keep=ids_keep
            )
        else:
            vision_embeds = None
        if input_ids is not None and inputs_embeds is not None:
            ...
            seg_data["tokens"]["bboxes"] = torch.zeros(
        ...
        **kwargs,
        ...
        if encoder_outputs.vision_embeds is not None:
            ...
            hidden_states = torch.cat(
                [encoder_outputs.last_hidden_state, vision_embeds], 1
            )
