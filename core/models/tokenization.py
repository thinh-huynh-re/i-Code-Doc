...
class UdopTokenizer(T5Tokenizer):
        ...
        **kwargs,
        ...
        if extra_ids > 0 and not "<extra_id_0>" in additional_special_tokens:
            ...
            additional_special_tokens.extend(
                ["<extra_l_id_{}>".format(i) for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                ["</extra_l_id_{}>".format(i) for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                ["<extra_t_id_{}>".format(i) for i in range(extra_ids)]
            )
            additional_special_tokens.extend(
                ["</extra_t_id_{}>".format(i) for i in range(extra_ids)]
            )
        ...
        if loc_extra_ids > 0 and not "<loc_0>" in additional_special_tokens:
            additional_special_tokens.extend(
                ["<loc_{}>".format(i) for i in range(loc_extra_ids)]
            )

        if other_extra_ids > 0 and not "<other_0>" in additional_special_tokens:
            additional_special_tokens.extend(
                ["<other_{}>".format(i) for i in range(other_extra_ids)]
            )
    ...
    def _convert_token_to_id(self, token):
        ...
        if token.startswith("<extra_id_"):
            ...
            num = int(match.group(1))
            return (
                self.vocab_size - num - 1 - self._other_extra_ids - self._loc_extra_ids
            )
...
class UdopConverter(SpmConverter):
    def vocab(self, proto):
        ...
        vocab += [
            ("<extra_l_id_{}>".format(i), 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]
        vocab += [
            ("</extra_l_id_{}>".format(i), 0.0)
            for i in range(num_extra_ids - 1, -1, -1)
        ]
        vocab += [
            ("<extra_t_id_{}>".format(i), 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]
        vocab += [
            ("</extra_t_id_{}>".format(i), 0.0)
            for i in range(num_extra_ids - 1, -1, -1)
        ]

        num_loc_extra_ids = self.original_tokenizer._loc_extra_ids
        vocab += [
            ("<loc_{}>".format(i), 0.0) for i in range(num_loc_extra_ids - 1, -1, -1)
        ]

        num_other_extra_ids = self.original_tokenizer._other_extra_ids
        vocab += [
            ("<other_0{}>".format(i), 0.0)
