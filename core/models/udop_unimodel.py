...
def collate_vlembed(
    ...
    ocr_points_x = torch.clip(
        torch.floor((seg_data[:, :, 0] + seg_data[:, :, 2]) / 2.0 * L).long(), 0, L - 1
    )
    ocr_points_y = (
        torch.clip(
            torch.floor((seg_data[:, :, 1] + seg_data[:, :, 3]) / 2.0 * L).long(),
            0,
            L - 1,
        )
        * L
    ...
    input_vision_patches = [
        inputs_patches[i][patch_inds[i]] for i in range(len(patch_inds))
    ]
    visual_segdata = [visual_segdata[i][patch_inds[i]] for i in range(len(patch_inds))]
    if attention_mask is not None:
        visual_attention_mask = [
            torch.tensor([1] * len(item)).to(attention_mask) for item in visual_segdata
        ]
    ...
    inputs_vision_patches = torch.stack(
        [
            pad_sequence(item, max_len, torch.zeros_like(inputs_patches[0, 0]))
            for item in input_vision_patches
        ]
    )
    visual_segdata = torch.stack(
        [
            pad_sequence(item, max_len, torch.zeros_like(seg_data[0, 0]))
            for item in visual_segdata
        ]
    )
    if attention_mask is not None:
    ...
    def __init__(self, config, embed_tokens=None):
        ...
        if self.is_decoder:
            self.num_layers = (
                ...
                if config.truncate_decoder_after_layer
                else config.num_layers
            )
        else:
            self.num_layers = (
                config.truncate_encoder_after_layer
                if config.truncate_encoder_after_layer
                else config.num_layers
            )
        ...
        if not self.is_decoder:
            self.cell2dembedding = CellEmbeddings(
...
class UdopUnimodelForConditionalGeneration(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self._max_decoder_length = (
            config.max_decoder_length if hasattr(config, "max_decoder_length") else 256
        )
        ...
        if encoder_outputs is None:
            ...
            if image is not None:
                ...
                if ids_keep is not None:
                    x = torch.gather(
                        x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, x.size(-1))
                    )
                    pad_tokens = self.pad_token.repeat(
